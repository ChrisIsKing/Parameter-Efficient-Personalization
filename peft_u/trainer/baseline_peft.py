...
def parse_args():
    ...
    out.test_parser.add_argument("--use_rag", type=bool, required=False, default=False)
...
def _get_dataset_and_users_it(
        ...
        uid_start_from: Union[str, int] = None, uid_end_at: Union[str, int] = None, seed: int = None, use_user_profile: bool = False, is_generative: bool = False, max_example_count: int = 3, use_rag: bool = False
) -> Tuple[Dict[str, InputEgDataset], List[str]]:
    # from peft_u._dset_uid_too_small import uid_too_small

    dset = load_dataset_with_prompts(dataset_name=dataset_name, leakage=leakage, seed=seed, use_user_profile=use_user_profile, max_example_count=max_example_count, use_rag=use_rag)
...
if __name__ == '__main__':
    ...
    def command_prompt():
        ...
        if cmd == 'train':
            ...
            dset, it = _get_dataset_and_users_it(**load_args, uid_start_from=strt, use_user_profile=use_user_profile, is_generative=is_generative, max_example_count=max_example_count, use_rag=use_rag)
            ...
            dset, it = _get_dataset_and_users_it(**load_args, uid_start_from=strt, is_generative=is_generative, max_example_count=max_example_count, use_rag=use_rag)
            n_user = len(it)
            ...
            for i, uid in enumerate(it, start=1):
                ...
                if not zeroshot:
                    ...
                    gc.collect()
            out_args = dict(d_accs=accs, logger_fl=logger_fl, eval_output_path=eval_output_path, is_generative=is_generative, max_example_count=max_example_count, use_rag=use_rag)
