...
def _get_dataset_and_users_it(
    ...
    dset = load_dataset_with_prompts(dataset_name=dataset_name, dataset_path=dataset_path, leakage=leakage, seed=seed, use_user_profile=True)
    ...
    def command_prompt():
        ...
        if cmd == 'train':
            ...
            for i, uid in enumerate(it, start=1):
                ...
                #     continue
                # torch.cuda.max_split_size_mb = 64
            ...
            if zeroshot:
                ...
                eval_out_str = f'{date}_ZeroShot-UserProfile-Eval_{pl.pa(d)}'
            else:
                eval_out_str = f'{model_name_or_path}_UserProfile-Eval-{date}'
